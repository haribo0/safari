	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4 && xhr.status == 200){
			...
			response.list.forEach(function(dto){
				...
				col1Div.textContent = dto.chatMsg[dto.chatMsg.length-1].msg;
...
function getChatMsg(dto) {
	...
	// 현재 누른 문의에 배경색을 주기 
	// 전에 선택한 row가 null이 아니면 
	/* if(liveChatId!=null && dto.id!=liveChatId) {
		const row1 = document.getElementById("row-"+liveChatId);
		row1.classList.remove("bg-light");
	}
	 */
	liveChatId = dto.id;
