...
<!-- 						<div class="row mt-3"> 
							...
							<div class="row mt-3 ms-3 me-1 mb-3"> 
					...
					<div class="card mb-3" style="padding: 8px;">
						<div class="card-body">
					  	<div class="row">
						  	...
						  	<div class="col d-grid ms-3 mt-1">
						  		...
						  		<div class="col">
									...
									<span class="text-align-center mb-1">${recruitDto.recruitDto.title}<br></span>
										...
										<%-- 위치 --%>
										<span class="text-secondary mt-1 ms-1" id="locationName_${recruitDto.recruitDto.id}"></span>
						    ...
						    <div class="col-1 d-flex align-items-start justify-content-end me-3" style="font-size: 20px;">
						    	<%-- <i class="bi bi-eye me-2"></i> <span class="me-3" style="font-size: 15px;">${recruitDto.recruitDto.views}</span>  --%>
						    	 ...
						    	 <%-- d-day --%>
						    	 <span id="dDayLabel_${recruitDto.recruitDto.id}" class="badge badge-lg mt-2 me-2" style="font-size: 12px; position: relative; top: -3px; color: #0095ff; background-color: #def6ff; border: ;"></span>
						    	 <%-- <fmt:formatDate value="${recruitDto.recruitDto.reg_date}" pattern="yyyy.MM.dd"/> <br>
						    	 <fmt:formatDate value="${recruitDto.recruitDto.end_date}" pattern="yyyy.MM.dd"/> <br>--%>
						    	 

								    <script>
								        // 페이지 로딩 시점에 D-day 계산 및 표시
								        var endDateStr = "<fmt:formatDate value='${recruitDto.recruitDto.end_date}' pattern='yyyy-MM-dd'/>";
								        var endDate = new Date(endDateStr);
								        var today = new Date();
								
								        // 종료 날짜와 오늘 날짜의 차이를 계산하여 D-day 계산
								        var timeDiff = endDate.getTime() - today.getTime();
								        var daysRemaining = Math.ceil(timeDiff / (1000 * 60 * 60 * 24)) - 1;
								
								        // D-day를 표시하는 부분
								        var dDayLabel = document.getElementById("dDayLabel_${recruitDto.recruitDto.id}");
								        console.log("d-day");
								        console.log(daysRemaining);
								        if (daysRemaining > 0) {
								        	
								            dDayLabel.innerText = "D-" + daysRemaining;
								        } else if (daysRemaining === 0) {
								            dDayLabel.innerText = "D-day";
								        } else {
								            //dDayLabel.innerText = "D+" + Math.abs(daysRemaining);
								            dDayLabel.innerText = "채용마감";
								            document.getElementById("dDayLabel_${recruitDto.recruitDto.id}").style.color = "#eb4034";
								        	document.getElementById("dDayLabel_${recruitDto.recruitDto.id}").style.borderColor = "";
								        	document.getElementById("dDayLabel_${recruitDto.recruitDto.id}").style.backgroundColor = "#ffd3cf";
								        }
								    </script>
						    	 <%-- d-day --%>
