	...
	public String onKakaopayProcess(HttpSession session, HttpServletResponse response, int id, String pg_token) {
		...
		try {
			...
			try {
				...
				//RentalOrderKakaopay rentalOrderKakaopay = objectMapper.readValue(input, RentalOrderKakaopay.class);
		...
		/*
		 * String redirectUri = "../user/myOrderListPage"; try {
		 * response.sendRedirect(redirectUri); } catch (IOException e) { // TODO
